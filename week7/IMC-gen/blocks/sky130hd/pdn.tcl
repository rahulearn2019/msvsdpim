...
add_global_connection -net {VSS} -pin_pattern {VVSS} -ground
...
add_pdn_stripe -grid {grid} -layer {met4} -starts_with POWER -width {1.2} -pitch {25} -offset {2} -extend_to_core_ring
add_pdn_stripe -grid {grid} -layer {met5} -starts_with POWER -width {1.6} -pitch {28} -offset {2} -extend_to_core_ring
...
####################################
#The below PDN is for PLL. a design whose pdn.tcl was used
if 0 {
...
define_pdn_grid -name {CORE_macro_grid_2} -voltage_domains {CORE} -macro -orient {R0 R180 MX MY} -halo {2.0 2.0 2.0 2.0} -default -grid_over_boundary -obstructions {li1 met1 met2 met3 met4}
